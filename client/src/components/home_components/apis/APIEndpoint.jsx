      ...
      <div className="rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-colors duration-150 p-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            <span className={`px-2 py-1 rounded text-[10px] font-sans font-bold border ${getMethodColor(endpoint.method)}`}>
              {endpoint.method}
            </span>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-white text-xs font-sans font-semibold">{endpoint.calls}</div>
              <div className="text-white/30 text-[8px]">Calls</div>
            </div>
            <div className="text-right">
              <div className="text-white text-xs font-sans font-semibold">{endpoint.avgResponse}</div>
              <div className="text-white/30 text-[8px]">Response</div>
            </div>
            <span className={`text-[9px] font-sans font-semibold ${getStatusColor(endpoint.status)}`}>
              ● {endpoint.status}
            </span>
          </div>
        </div>
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black p-5">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
        ...
        <span className={`text-[10px] font-sans font-semibold ${getStatusColor(endpoint.status)}`}>
          ● {endpoint.status}
        </span>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <h4 className="text-white font-sans text-[11px] font-bold mb-2">Request Parameters</h4>
        ...
        <div>
          <h4 className="text-white font-sans text-[11px] font-bold mb-2">Example Response</h4>
