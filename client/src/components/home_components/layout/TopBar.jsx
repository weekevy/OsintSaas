  ...
  const TopBar = ({ 
    ...
    return (
      <header className="flex flex-col bg-[#0a0a0a] border-b border-white/[0.06] relative z-[9998]" style={{ fontFamily: "'Plus Jakarta Sans', 'Inter', sans-serif" }}>
        ...
        {/* Top Row */}
        <div className="flex items-center justify-between px-5 sm:px-7 lg:px-10 py-4 gap-4">
          ...
          <div className="flex items-center gap-6 flex-1">
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <h1 className="font-black text-2xl md:text-3xl text-white whitespace-nowrap tracking-[-0.02em]">
                Weekey<span className="bg-gradient-to-r from-[#00E5FF] to-[#2DD4BF] bg-clip-text text-transparent">Osint</span>
              </h1>
            </div>

            {/* Desktop Search Bar */}
            <div className="hidden sm:flex flex-1 max-w-2xl relative z-[9999]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              <button className="px-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:border-[#00E5FF]/30 transition-all flex items-center gap-2.5">
            ...
            <div className="relative" ref={notificationsRef}>
              <button onClick={() => setNotificationsOpen(!notificationsOpen)} className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] text-white/50 hover:text-[#00E5FF] hover:border-[#00E5FF]/30 transition-all relative">
        ...
        {/* Navigation Bar */}
        <div className="px-5 sm:px-7 lg:px-10 border-t border-white/[0.04]">
            ...
            {navItems.map((item) => {
              ...
              return (
                <button key={item.id} onClick={() => onTabChange(item.id)} className="relative group py-2.5">
                  <div className={`flex items-center gap-2.5 px-5 sm:px-6 rounded-lg transition-all duration-200 py-2 ${
                    isActive 
                      ? 'text-[#00E5FF] bg-[#00E5FF]/8' 
                      : 'text-white/35 hover:text-white hover:bg-white/[0.03]'
                  }`}>
                    <span className="text-current w-5 h-5 transition-transform duration-200 group-hover:scale-110">{item.icon}</span>
                    <span className="font-bold text-xs uppercase tracking-wider hidden sm:inline">{item.label}</span>
                  </div>
                  {isActive && (
                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-[2px] bg-gradient-to-r from-[#00E5FF] to-[#2DD4BF] rounded-full" />
                  )}
                </button>
              );
            })}
