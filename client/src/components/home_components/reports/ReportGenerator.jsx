      ...
      <div className="space-y-5 font-sans">
          ...
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex items-center flex-1 last:flex-none">
              <div className={`flex flex-col items-center ${i < 4 ? 'flex-1' : ''}`}>
                <div className={`w-7 h-7 border flex items-center justify-center text-[10px] font-sans font-bold
                  ${step >= i 
                ...
                <span className="text-[7px] font-sans uppercase tracking-[0.12em] mt-1 text-white/40">
                  {i === 1 && 'TEMPLATE'}
                  {i === 2 && 'CONTENT'}
                  {i === 3 && 'FORMAT'}
                  {i === 4 && 'SCHEDULE'}
              ...
              {i < 4 && (
                <div className={`h-[1px] flex-1 mx-2 ${
                  step > i ? 'bg-[#00E5FF]/50' : 'bg-white/10'
                }`} />
              )}
            </div>
          ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              {templates.map((template) => (
                ...
                >
                  ...
                  <span className="text-[9px] font-sans font-bold text-white uppercase tracking-[0.08em]">{template.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              {sections.map((section) => (
                <label key={section.id} className="flex items-center gap-3 p-2 cursor-pointer border border-transparent hover:border-[#00E5FF]/20 transition-all">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-white/60 hover:text-white text-[9px] font-sans uppercase tracking-[0.08em]">{section.name}</span>
                </label>
              ))}
            </div>
            ...
            <div className="mt-3">
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-[#0d1114] border border-white/10 text-white text-[9px] font-sans uppercase tracking-[0.08em] focus:outline-none focus:border-[#00E5FF]/50 transition-colors appearance-none cursor-pointer"
                style={{
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                >
                  ...
                  <span className="text-white text-[8px] font-sans font-bold uppercase tracking-[0.08em] block">{format.name}</span>
                  <span className="text-white/30 text-[6px] font-sans uppercase tracking-[0.08em] mt-0.5 block">{format.description}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                {[
                  ...
                  <label key={option.id} className="flex items-center gap-3 p-2 cursor-pointer border border-transparent hover:border-[#00E5FF]/20 transition-all">
                    <input
                      type="radio"
                      name="schedule"
                      value={option.id}
                      ...
                      className="w-3.5 h-3.5 border border-white/20 bg-transparent text-[#00E5FF] focus:ring-0 focus:ring-offset-0"
                    />
                    <span className="text-white/60 hover:text-white text-[9px] font-sans uppercase tracking-[0.08em]">{option.label}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {formData.schedule !== 'now' && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[#0d1114] border border-white/10 text-white text-[9px] font-sans placeholder:text-white/20 focus:outline-none focus:border-[#00E5FF]/50 transition-colors"
                />
                <p className="text-white/30 text-[7px] font-sans uppercase tracking-[0.08em] mt-1">
        ...
        <div className="flex gap-3 pt-4 border-t border-white/10">
          {step > 1 && (
            <button
              onClick={handleBack}
              className="px-4 py-2 border border-white/10 text-white/50 hover:text-white hover:border-white/20 transition-all text-[9px] font-sans uppercase tracking-[0.08em]"
            >
              BACK
            </button>
          )}
          ...
          {step < 4 ? (
            <button
              onClick={handleNext}
              className="px-5 py-2 border border-[#00E5FF]/30 bg-[#00E5FF]/10 text-[#00E5FF] hover:bg-[#00E5FF]/20 transition-all text-[9px] font-sans uppercase tracking-[0.08em]"
            >
              NEXT
            </button>
          ) : (
