...
const CurrentModules = ({
  ...
  const getStatusConfig = useCallback((status) => {
    const configs = {
      running:   { color: 'text-[#00E5FF]', bg: 'bg-[#00E5FF]/10', border: 'border-[#00E5FF]/35', label: 'ACTIVE' },
      queued:    { color: 'text-[#a78bfa]', bg: 'bg-[#a78bfa]/10', border: 'border-[#a78bfa]/35', label: 'QUEUED' },
      pending:   { color: 'text-[#fbbf24]', bg: 'bg-[#fbbf24]/10', border: 'border-[#fbbf24]/35', label: 'PENDING' },
      paused:    { color: 'text-[#fb923c]', bg: 'bg-[#fb923c]/10', border: 'border-[#fb923c]/35', label: 'PAUSED' },
      completed: { color: 'text-[#22d3ee]', bg: 'bg-[#22d3ee]/10', border: 'border-[#22d3ee]/35', label: 'COMPLETE' },
      failed:    { color: 'text-[#f87171]', bg: 'bg-[#f87171]/10', border: 'border-[#f87171]/35', label: 'FAILED' },
      stopped:   { color: 'text-white/35', bg: 'bg-white/5', border: 'border-white/10', label: 'STOPPED' },
    };
  ...
  const ModuleIcon = useCallback(({ type }) => {
    ...
    const iconMap = {
      linkedin: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M20.45 20.45h-3.55v-5.57c0-1.33-.03-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.48-.9 1.64-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.45v6.29zM5.34 7.43a2.06 2.06 0 1 1 0-4.12 2.06 2.06 0 0 1 0 4.12zm1.78 13.02H3.56V9h3.56v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.73V1.73C24 .77 23.2 0 22.22 0z"/></svg>,
      job: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M20 6h-4V4c0-1.1-.9-2-2-2h-4c-1.1 0-2 .9-2 2v2H4c-1.1 0-2 .9-2 2v11c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-6 0h-4V4h4v2z"/></svg>,
      social: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92c0-1.61-1.31-2.92-2.92-2.92z"/></svg>,
      website: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>,
      email: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>,
      'email-scam': <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>,
      ...
      crypto: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z"/></svg>,
    };
    return (
      <div className={`w-[38px] h-[38px] rounded-lg ${tc.bg} border ${tc.border} flex items-center justify-center flex-shrink-0 ${tc.color} transition-all`}>
  ...
  const StatusBadge = useCallback(({ status }) => {
    ...
    const icons = {
      running:   <span className="w-1.5 h-1.5 rounded-full bg-current inline-block flex-shrink-0" />,
      queued:    <span className="w-1.5 h-1.5 rounded-full bg-current inline-block flex-shrink-0" />,
      pending:   <span className="w-1.5 h-1.5 rounded-full bg-current inline-block flex-shrink-0" />,
      completed: <span className="w-1.5 h-1.5 rounded-full bg-current inline-block flex-shrink-0" />,
      failed:    <span className="w-1.5 h-1.5 rounded-[1px] bg-current inline-block flex-shrink-0" />,
      paused:    <span className="w-1.5 h-1.5 rounded-full bg-current inline-block flex-shrink-0" />,
      stopped:   <span className="w-1.5 h-1.5 rounded-full bg-current inline-block flex-shrink-0" />,
    };
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-[3px] rounded bg-opacity-10 ${cfg.bg} ${cfg.color} border ${cfg.border} text-[8px] font-bold tracking-widest whitespace-nowrap flex-shrink-0`}>
  ...
  const LoadingSkeleton = () => (
    <div className="glass-card rounded-xl overflow-hidden font-sans w-full flex flex-col relative min-h-[364px] max-h-[420px] border-white/5">
      <div className="p-[14px_16px_12px] border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-[#00E5FF]/5 border border-[#00E5FF]/10" />
          <div>
            <div className="w-[120px] h-3 rounded bg-white/5 animate-pulse mb-1.5" />
            <div className="w-[70px] h-2 rounded bg-white/5 animate-pulse" />
          </div>
        </div>
      </div>
      <div className="p-[10px_14px] grid grid-cols-4 gap-[7px] min-h-[74px]">
        {[1,2,3,4].map(i => <div key={i} className="h-[52px] rounded-md bg-white/5 animate-pulse" />)}
      </div>
      <div className="p-[8px_10px] flex flex-col gap-1.5 min-h-[180px]">
        {[1,2,3].map(i => <div key={i} className="h-[72px] rounded-lg bg-white/5 border border-white/5 animate-pulse" />)}
      </div>
    </div>
  );
  ...
  const ModuleItem = useCallback(({ module, isSelected }) => {
    ...
    return (
      <div
        ...
        className={`rounded-xl border transition-all cursor-pointer flex gap-3 items-start relative overflow-hidden p-3 ${
          isSelected 
            ? `border-[#00E5FF]/40 bg-gradient-to-br from-[#00E5FF]/10 to-transparent` 
            : isHovered ? 'border-white/10 bg-white/5' : 'border-white/5 bg-white-[0.015]'
        }`}
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1">
            <span className={`text-[11px] font-bold tracking-wider uppercase truncate transition-colors ${isSelected ? 'text-[#00E5FF]' : 'text-white'}`}>
              {module.name}
            </span>
            <StatusBadge status={module.status} />
          </div>

          <p className={`text-[9px] truncate mb-2 transition-colors ${isSelected ? tc.color : 'text-white/40'}`}>
            {module.target}
          </p>

          {showProgress && (
            <div className="mb-2">
              <div className="flex justify-between text-[8px] text-white/35 tracking-widest mb-1">
                <span>PROGRESS</span>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[8px] text-white/30 flex items-center gap-1">
              <svg className="w-2 h-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12,6 12,12 16,14"/></svg>
              {module.startTime}
            </span>
            {module.findings > 0 && (
              <span className={`px-[7px] py-[3px] rounded text-[8px] tracking-wider font-bold ${
                module.findings > 10 
  ...
  const statCells = [
    { label: 'ACTIVE',   value: moduleStats.active,    color: 'text-[#00E5FF]', bg: 'bg-[#00E5FF]/5', border: 'border-[#00E5FF]/20' },
    { label: 'PENDING',  value: moduleStats.pending,   color: 'text-[#fbbf24]', bg: 'bg-[#fbbf24]/5', border: 'border-[#fbbf24]/20' },
    { label: 'DONE',     value: moduleStats.completed, color: 'text-[#22d3ee]', bg: 'bg-[#22d3ee]/5', border: 'border-[#22d3ee]/20' },
    { label: 'FAILED',   value: moduleStats.failed,    color: 'text-[#f87171]', bg: 'bg-[#f87171]/5', border: 'border-[#f87171]/20' },
  ];

  const hasActiveModules = modules.length > 0;

  return (
    <div className={`glass-card rounded-xl overflow-hidden font-sans w-full flex flex-col relative min-h-[364px] max-h-[420px] ${hasActiveModules ? 'border-[#00E5FF]/25' : 'border-white/5'}`}>
      <div className="absolute inset-0 pointer-events-none opacity-[0.025]" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, white 1px, transparent 0)', backgroundSize: '22px 22px' }} />

      <div className="p-[14px_16px_12px] border-b border-white/5 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-[11px]">
          <div className="w-9 h-9 rounded-lg bg-[#00E5FF]/10 border border-[#00E5FF]/20 flex items-center justify-center flex-shrink-0">
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-[#00E5FF]">
              <path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/>
            </svg>
          </div>
          <div>
            <h3 className="text-xs font-bold text-white tracking-wider uppercase m-0">ACTIVE PROJECTS</h3>
            <p className="text-[9px] text-white/30 tracking-widest uppercase mt-0.5">REAL-TIME MONITOR</p>
          </div>
        </div>
        <div className="flex gap-1.5 items-center">
          <div className="flex items-center gap-1.5 p-[4px_10px] rounded bg-[#00E5FF]/10 border border-[#00E5FF]/20 mr-1">
            <span className="w-1.5 h-1.5 rounded-full bg-[#00E5FF] inline-block" />
            <span className="text-[8px] text-[#00E5FF] tracking-[0.12em] font-bold">LIVE</span>
          </div>
          <button onClick={forceRefresh} disabled={loading} className="w-[30px] h-[30px] border border-white/10 rounded-md bg-transparent text-white/30 flex items-center justify-center transition-all hover:border-[#00E5FF]/40 hover:text-[#00E5FF] p-0">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
              className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`}>
              <path d="M4 4v5h5M20 20v-5h-5M4.55 9A8 8 0 0 1 20 12M19.45 15A8 8 0 0 1 4 12"/>
            </svg>
          </button>
          <button onClick={handleViewAll} className="w-[30px] h-[30px] border border-white/10 rounded-md bg-transparent text-white/30 flex items-center justify-center transition-all hover:border-[#00E5FF]/40 hover:text-[#00E5FF] p-0">
      ...
      <div className="grid grid-cols-4 p-[10px_14px] gap-[7px] border-b border-white/5 flex-shrink-0 min-h-[74px]">
        {statCells.map(s => (
          <div key={s.label} className={`rounded-md p-[8px_6px] text-center ${s.bg} border ${s.border}`}>
            <div className={`text-2xl font-bold ${s.color} leading-none tabular-nums`}>{s.value}</div>
            <div className="text-[8px] text-white/35 tracking-wider mt-1">{s.label}</div>
          </div>
        ))}
      </div>

      <div className="p-2 flex flex-col gap-1.5 min-h-[180px] max-h-[280px] overflow-y-auto scrollbar-thin scrollbar-thumb-[#00E5FF]/20">
        {modules.length === 0 && !loading ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 gap-2 min-h-[160px]">
            <div className="w-10 h-10 border border-white/5 rounded-lg flex items-center justify-center">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-white/15">
                <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14z"/>
              </svg>
            </div>
            <p className="text-[11px] font-bold text-white/40 tracking-wider uppercase m-0">NO ACTIVE PROJECTS</p>
            <p className="text-[9px] text-white/20 tracking-wider uppercase m-0">START A SCAN TO BEGIN</p>
          </div>
        ) : (
        ...
        {loading && !initialLoad && (
          <div className="flex justify-center p-2.5">
            <div className="w-3.5 h-3.5 border-2 border-[#00E5FF]/20 border-t-[#00E5FF] rounded-full animate-spin" />
          </div>
        )}
      </div>
    </div>
