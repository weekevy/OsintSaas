      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <span className="text-[8px] sm:text-[10px] text-white/30 flex items-center gap-1.5 font-sans">
            ...
            {module.findings > 0 && (
              <span className={`px-2 py-1 rounded text-[8px] sm:text-[10px] font-bold ${module.findings > 10 ? 'bg-red-500/15 text-red-400' : 'bg-yellow-500/15 text-yellow-400'}`}>{module.findings} FINDINGS</span>
            )}
          </div>
        </div>
        <div className={`absolute right-3 top-1/2 -translate-y-1/2 transition-opacity ${isSelected ? 'opacity-100' : 'opacity-0'}`}><svg className="w-4 h-4 text-[#00E5FF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7"/></svg></div>
      </div>
    );
