  ...
  const renderLoginForm = () => (
    <div className="p-10">
      <div className="text-center mb-8">
        ...
        <p className="text-white/40 text-sm font-medium">Access your intelligence dashboard</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button 
          type="submit" 
          disabled={isLoading} 
          className="w-full py-4 bg-white text-black font-bold rounded-xl hover:bg-gray-200 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
        >
      ...
      <div className="relative my-8">
        ...
        <div className="relative flex justify-center text-[10px]"><span className="px-4 bg-black text-white/20 font-bold uppercase tracking-widest">Secure Connect</span></div>
      </div>

      <div className="grid grid-cols-2 gap-4 mb-8">
        <button onClick={googleLogin} className="flex items-center justify-center gap-2 py-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all">
        ...
        <button onClick={githubLogin} className="flex items-center justify-center gap-2 py-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all">
          ...
          <span className="text-xs font-bold text-white/60">GitHub</span>
        </button>
      </div>

      <p className="text-center text-xs font-bold text-white/20 uppercase tracking-widest">
        New Operator?{' '}
        <button onClick={onSwitchToRegister} className="text-[#00E5FF] hover:text-[#00D4EB] transition-colors">Apply for Access</button>
      </p>
    </div>
  );
  ...
  const renderOTPForm = () => (
    <div className="p-10">
      <div className="text-center mb-8">
        ...
        <p className="text-white/40 text-sm font-medium">Identity check for <span className="text-[#00E5FF]">{tempUserEmail}</span></p>
      </div>
      ...
      <form onSubmit={handleOTPSubmit} className="space-y-8">
        ...
        <div className="text-center">
          {canResend ? (
            ...
            <p className="text-xs font-bold uppercase tracking-widest text-white/20">Available in <span className="text-[#00E5FF]">{timeLeft}s</span></p>
          )}
        </div>
