...
const RegisterModal = ({ isOpen, onClose, onSwitchToLogin }) => {
  ...
  const handleOTPSubmit = async (e, providedOtp = null) => {
    ...
    if (otpValue === '000000') {
      ...
      setOtpError('Validation failed. Resetting registration...');
      // After a short delay, go back to register form as requested
      setTimeout(() => {
        setShowOTP(false);
        setOtpCode(['', '', '', '', '', '']);
        setOtpError('');
        setError('Email verification failed. Please start over.');
      }, 1500);
    }
    setIsLoading(false);
  };
    ...
    <div className="p-6 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-white/50 text-[10px] font-bold uppercase tracking-widest mb-2">New Email Address</label>
            <input 
              type="email" 
              required 
          ...
          <div>
            <label className="block text-white/50 text-[10px] font-bold uppercase tracking-widest mb-2">Unique Username</label>
            <input 
              type="text" 
              required 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-white/50 text-[10px] font-bold uppercase tracking-widest mb-2">Create Password</label>
            <input 
              type="password" 
              required 
          ...
          <div>
            <label className="block text-white/50 text-[10px] font-bold uppercase tracking-widest mb-2">Confirm Password</label>
            <input 
              type="password" 
              required 
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-[10px]"><span className="px-4 bg-[#0a0a0a] text-white/20 font-bold uppercase tracking-widest">Or continue with</span></div>
      </div>

      <div className="space-y-2 mb-6">
        <button 
          onClick={googleLogin} 
          className="w-full flex items-center justify-center gap-3 py-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors duration-200"
        >
      ...
      <div className="text-center">
        <p className="text-xs font-bold text-white/20 uppercase tracking-widest">
          Already Enrolled?{' '}
          <button onClick={onSwitchToLogin} className="text-[#00E5FF] hover:text-[#00D4EB] transition-colors duration-200 inline-block">
            Sign In
          </button>
        </p>
      </div>
  ...
  const renderOTPForm = () => (
    <div className="p-6 md:p-8">
      ...
      <form onSubmit={handleOTPSubmit} className="space-y-6">
        ...
        <div className="text-center">
          {canResend ? (
            <button type="button" onClick={() => setTimeLeft(60)} className="text-xs font-bold uppercase tracking-widest text-[#00E5FF] transition-colors duration-200">
              Resend Code
            </button>
          ) : (
