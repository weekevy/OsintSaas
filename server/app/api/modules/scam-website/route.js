  ...
  try {
    ...
    const [scans] = await query(`
      SELECT 
        ...
        sw.expiry_date,
        sw.registrant_country,
        sw.is_private_registration,
        sw.notes,
    ...
    const formattedScans = scans.map(scan => ({
      ...
      assets: {
        ...
        red_flags_list: scan.red_flags_list,
        fake_reviews: scan.fake_reviews,
        payment_requests: scan.payment_requests,
        registrar: scan.registrar,
        registration_date: scan.registration_date,
        expiry_date: scan.expiry_date,
        registrant_country: scan.registrant_country,
        is_private_registration: scan.is_private_registration,
        notes: scan.notes
      },
...
// POST - Create new scam website scan
export async function POST(request) {
  ...
  try {
    ...
    const requestId = `scam_website_${user.id}_${websiteUrl}`;
    ...
    try {
      ...
      if (!projectId) {
        ...
        if (projects.length > 0) projectId = projects[0].id;
          ...
          const [newProject] = await connection.execute(`INSERT INTO projects (user_id, name, description, status, created_at) VALUES (?, 'Default Project', 'Default project for website scans', 'active', NOW())`, [user.id]);
          projectId = newProject.insertId;
        }
      }

      const [targetResult] = await connection.execute(`INSERT INTO targets (project_id, type, value, label, status, created_at) VALUES (?, ?, ?, ?, ?, NOW())`, [projectId, 'url', websiteUrl, body.domain_name || websiteUrl, 'pending']);
      const targetId = targetResult.insertId;

      const [scanResult] = await connection.execute(`INSERT INTO scans (target_id, scan_type, status, priority, progress, created_at) VALUES (?, ?, ?, ?, 0, NOW())`, [targetId, 'scam-website', 'queued', 1]);
      const scanId = scanResult.insertId;

      await connection.execute(`
        INSERT INTO scam_website_scans (
          scan_id, website_url, domain_name, ip_address, 
          red_flags_list, fake_reviews, payment_requests, 
          registrar, registration_date, expiry_date, 
          registrant_country, is_private_registration, notes, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`, 
        [
          scanId, 
          websiteUrl, 
          ...
          body.red_flags_list || null, 
          body.fake_reviews || null, 
          body.payment_requests || null, 
          body.registrar || null, 
          body.registration_date || null, 
          body.expiry_date || null, 
          body.registrant_country || null, 
          body.is_private_registration || null, 
          body.notes || null
        ]
      );
      ...
      return NextResponse.json({ 
        success: true, 
        message: '✅ Website scan created successfully!', 
        scan: { id: scanId, status: 'queued', website_url: websiteUrl } 
      });
    } catch (error) {
...
export async function PATCH(request) {
  ...
  try {
    ...
    console.error('Error updating scam website scan status:', error);
...
export async function DELETE(request) {
  ...
  try {
    ...
    connection = await pool.getConnection();
    await connection.beginTransaction();
    await connection.execute('DELETE FROM scam_website_scans WHERE scan_id = ?', [id]);
    ...
    await connection.commit();
    connection.release();
    return NextResponse.json({ success: true, message: '✅ Website scan deleted successfully' });
  } catch (error) {
    if (connection) connection.release();
    console.error('Error deleting scam website scan:', error);
  ...
  try {
    ...
    await pool.execute(`
      UPDATE scam_website_scans SET 
        website_url = ?, domain_name = ?, ip_address = ?, 
        red_flags_list = ?, fake_reviews = ?, payment_requests = ?, 
        registrar = ?, registration_date = ?, expiry_date = ?, 
        registrant_country = ?, is_private_registration = ?, 
    ...
    console.error('Error updating scam website scan:', error);
