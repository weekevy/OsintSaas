...
// GET - Fetch all email leak scans
  ...
  try {
    ...
    const formattedScans = scans.map(scan => ({
      ...
      assets: {
        ...
        breach_details: scan.breach_details
      },
...
export async function POST(request) {
  ...
  try {
    ...
    const requestId = `email_${user.id}_${emailAddress}`;
    ...
    try {
      ...
      if (!projectId) {
        ...
        if (projects.length > 0) projectId = projects[0].id;
          ...
          const [newProject] = await connection.execute(`INSERT INTO projects (user_id, name, description, status, created_at) VALUES (?, 'Default Project', 'Default project for email scans', 'active', NOW())`, [user.id]);
          projectId = newProject.insertId;
        }
      }

      const [targetResult] = await connection.execute(`INSERT INTO targets (project_id, type, value, label, status, created_at) VALUES (?, ?, ?, ?, ?, NOW())`, [projectId, 'email', emailAddress, `Email: ${emailAddress}`, 'pending']);
      const targetId = targetResult.insertId;

      const [scanResult] = await connection.execute(`INSERT INTO scans (target_id, scan_type, status, priority, progress, created_at) VALUES (?, ?, ?, ?, 0, NOW())`, [targetId, 'email-leak', 'queued', 1]);
      const scanId = scanResult.insertId;

      await connection.execute(`INSERT INTO email_leak_scans (scan_id, email_address, created_at) VALUES (?, ?, NOW())`, [scanId, emailAddress]);

      await connection.commit();
      connection.release();
      pendingRequests.delete(requestId);

      return NextResponse.json({ success: true, message: '✅ Email leak scan created successfully!', scan: { id: scanId, status: 'queued', email_address: emailAddress } });
    } catch (error) {
    ...
    if (connection) connection.release();
    console.error('Error creating email scan:', error);
...
export async function PATCH(request) {
  ...
  try {
    ...
    console.error('Error updating email scan status:', error);
...
export async function DELETE(request) {
  ...
  try {
    ...
    connection = await pool.getConnection();
    await connection.beginTransaction();
    await connection.execute('DELETE FROM email_leak_scans WHERE scan_id = ?', [id]);
    ...
    await connection.commit();
    connection.release();
    return NextResponse.json({ success: true, message: '✅ Email scan deleted successfully' });
  } catch (error) {
    if (connection) connection.release();
...
export async function PUT(request) {
  try {
    ...
    if (scanCheck.length === 0) return NextResponse.json({ error: 'Scan not found or unauthorized' }, { status: 404 });
    await pool.execute(`UPDATE email_leak_scans SET email_address = ?, additional_emails = ?, updated_at = NOW() WHERE scan_id = ?`, [body.email_address || null, body.additional_emails || null, id]);
    return NextResponse.json({ success: true, message: 'Scan updated successfully' });
  } catch (error) {
    console.error('Error updating email scan:', error);
