...
export async function POST(request) {
  ...
  try {
    ...
    const [newTarget] = await connection.execute(
      'INSERT INTO targets (project_id, type, value, status) VALUES (?, "url", ?, "active")',
      [projectId, target]
    );
    ...
    await connection.execute(
      'INSERT INTO job_recruitment_scans (scan_id, job_url, analysis_status) VALUES (?, ?, "pending")',
      [scanId, target]
    );
    ...
    return NextResponse.json({
      success: true,
      message: 'Analysis mission dispatched successfully',
      scan_id: scanId,
      job_id: job.id,
      target: target,
      status: 'dispatched',
    ...
    console.error('API v1 Analyze URL Error:', error);
