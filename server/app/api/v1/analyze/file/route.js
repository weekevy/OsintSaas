...
export async function POST(request) {
  ...
  try {
    ...
    if (!target) {
      return NextResponse.json({ error: 'File reference or name is required' }, { status: 400 });
    }
    ...
    const [newTarget] = await connection.execute(
      'INSERT INTO targets (project_id, type, value, status) VALUES (?, "file", ?, "active")',
      [projectId, target]
    );
    ...
    await connection.execute(
      'INSERT INTO job_recruitment_scans (scan_id, notes, analysis_status) VALUES (?, ?, "pending")',
      [scanId, `File analysis: ${target}`]
    );
    ...
    return NextResponse.json({
      success: true,
      message: 'File analysis mission dispatched successfully',
      scan_id: scanId,
      job_id: job.id,
      target: target,
      status: 'dispatched',
    ...
    console.error('API v1 Analyze File Error:', error);
